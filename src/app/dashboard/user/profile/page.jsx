...
export default function ProfilePage() {
  ...
  const handleImageUpload = async (e) => {
    ...
    try {
      ...
      if (!apiKey) {
        toast.error("ImgBB API Key missing in environment file");
  ...
  const handleUpdateProfile = async (e) => {
    ...
    try {
      ...
      const dbResponse = await fetch(`${base}/api/users/update-profile`, {
        method: "PUT",
      ...
      if (!dbResponse.ok) {
        throw new Error(dbData.error || "Failed to update native database logs");
      }
      ...
      await authClient.updateUser({
        ...
        onSuccess: async () => {
          toast.success("Profile records secured successfully!");
        ...
        onError: (ctx) => {
          toast.error(ctx.error.message || "Failed client session refresh");
          setIsUpdating(false);
        }
      });
    } catch (error) {
      console.error(error);
      toast.error(error.message || "Something went wrong during synchronization");
      setIsUpdating(false);
    }
  };
    ...
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      <div className="bg-[#243239] p-6 rounded-2xl border border-white/5 flex items-center justify-between">
        <div>
          ...
          <p className="text-white/40 text-sm mt-1">Update your personal information and profile picture securely.</p>
        </div>
        <div className="hidden sm:block">
          <div className="px-3 py-1 bg-[#df6742]/10 border border-[#df6742]/20 rounded-full">
            <span className="text-[#df6742] text-xs font-bold uppercase tracking-widest">{user?.role || "user"}</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-[#243239] p-6 rounded-2xl border border-white/5">
          <form onSubmit={handleUpdateProfile} className="space-y-5">
            ...
            >
              {isUpdating ? <Loader2 className="w-5 h-5 animate-spin" /> : <Save size={19} />}
